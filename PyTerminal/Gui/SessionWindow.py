...
class SessionsWindow(object):
    def __init__(self, parent_window, initial_session_row_count=1, initial_session_col_count=1):
        ...
        self.session_windows = [[SerialSession for y in range(initial_session_col_count)] for x in range(
            initial_session_row_count)]

    def build_window(self, sessions: list[SerialSession], session_row_count, session_col_count):
